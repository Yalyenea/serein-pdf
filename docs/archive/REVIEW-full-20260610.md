> **Archive** · 2026-08-05  
> Full engineering review (2026-06-10, progress sync 2026-07-10) including product/code highlights and completed §5.1 work.  
> Live open debt and roadmap: [REVIEW.md](../../REVIEW.md).
...
- **编译热重载**:外部编译器原地写入或原子替换(写临时文件再 rename)后自动重载,并优先恢复 PDFView 的实时页码;**有未保存批注的文档拒绝自动重载**,笔记永远不被编译产物冲掉。
...
- **浏览器式对比分屏**:split pair 记忆绑定的两个 PDF——普通点 tab 恢复或临时离开 pair,`Option` 激活才编辑 pane;支持**同一 PDF 两个独立视口对比**(独立页码/缩放)与左右 / 上下布局,且不产生幽灵 tab、不进历史不持久化。
- **多窗口 workspace**:每窗独立 tab 集 / 搜索 / 最近关闭栈;一键合并所有窗口,并通过菜单或 tab 拖拽把 PDF 移到新窗口 / 已有窗口。
...
- 阅读与功能快捷键可配:设置页录制、冲突拒绝、`none` 显式解绑;物理左 `Cmd+1/2/3` 固定负责前三个 tab,物理右 Command 继续执行配置中的数字阅读模式。
