...
final class AppUpdateServiceTests: XCTestCase {
    ...
    func testInstallAppFromDMGReplacesDestination() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("SereinUpdateTest-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let sourceApp = root.appendingPathComponent("payload/Serein.app", isDirectory: true)
        try FileManager.default.createDirectory(
            at: sourceApp.appendingPathComponent("Contents", isDirectory: true),
            withIntermediateDirectories: true
        )
        let marker = sourceApp.appendingPathComponent("Contents/version.txt")
        try "new-build".write(to: marker, atomically: true, encoding: .utf8)

        let staging = root.appendingPathComponent("dmg-src", isDirectory: true)
        try FileManager.default.createDirectory(at: staging, withIntermediateDirectories: true)
        try FileManager.default.copyItem(
            at: sourceApp,
            to: staging.appendingPathComponent("Serein.app", isDirectory: true)
        )

        let dmgURL = root.appendingPathComponent("Serein-test.dmg")
        let create = Process()
        ...
        XCTAssertEqual(create.terminationStatus, 0, "hdiutil create failed")

        let destination = root.appendingPathComponent("Applications/Serein.app", isDirectory: true)
        try FileManager.default.createDirectory(
            at: destination.appendingPathComponent("Contents", isDirectory: true),
            withIntermediateDirectories: true
        )
        try "old-build".write(
            to: destination.appendingPathComponent("Contents/version.txt"),
            atomically: true,
            encoding: .utf8
        )

        let service = AppUpdateService(currentVersion: "0.0.1")
        try service.installApp(fromDMG: dmgURL, to: destination)

        let installed = try String(contentsOf: destination.appendingPathComponent("Contents/version.txt"))
        XCTAssertEqual(installed, "new-build")
    }
}
