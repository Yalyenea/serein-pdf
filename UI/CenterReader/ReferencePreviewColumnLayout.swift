...
enum ReferencePreviewColumnLayout {
    static func columnBounds(characterBounds: [CGRect], pageBounds: CGRect, target: CGPoint) -> CGRect? {
        ...
        let candidates: [ClosedRange<CGFloat>] = rows.flatMap { row in
        ...
        for gap in candidates {
            ...
            let supporting = rows.filter { row in
                !crosses(row) && center - row[0].minX > width * 0.18
                    && row.last!.maxX - center > width * 0.18
            }
            guard supporting.count >= 3, supporting.count * 2 >= rows.count else { continue }
        ...
        let column = best.rows.flatMap { $0 }.filter { isLeft ? $0.midX < center : $0.midX > center }
