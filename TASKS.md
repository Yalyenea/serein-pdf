...
- Wave 1 M9 最近文件启动器：做 Spotlight 风格 recent-files palette，支持搜索、空格多选、回车打开、`?` 快捷键帮助。
- Wave 2 M10 预研：再做 `M10-001`，把扩展机制 RFC 先落出来。
...
- [ ] `M9-001` Spotlight 风格面板:`Cmd+Shift+Space` 拉起最近文件启动器,默认在当前窗口中心显示。
- [ ] `M9-002` 搜索与排序:基于 `recentDocumentURLs` 提供文件名/路径匹配,结果保持最近打开优先。
- [ ] `M9-003` 键盘交互:`↑` / `↓` 选中,`Space` 切换多选,`Enter` 打开选中或当前项,`Esc` 关闭。
- [ ] `M9-004` `?` 帮助:面板内切换快捷键说明,不跳出到外部文档。
- [ ] `M9-005` 测试:覆盖搜索过滤、多选打开、空列表与帮助切换。

## 8. Milestone 10:扩展生态(长期预研)

- [ ] `M10-001` 扩展机制 RFC:`docs/extensions-rfc.md` 列选型,至少比较进程内 Swift 插件 / URL scheme / 外部 CLI / WebKit 壳。
- [ ] `M10-002` PoC:若决策继续,选一条路径把"导出高亮"重写为插件,可在 app 中运行。
- [ ] `M10-003` Slate Extension API 草稿:面向未来扩展开发者。
- [ ] `M10-004` 若推迟,在 RFC 写清"为什么现在不做"(安全、上架、维护成本)。
...
- [ ] `UAT-33` 最近文件启动器内 `?` 能显示快捷键帮助并可关闭
- [ ] `UAT-34` 扩展机制 RFC 存在并评审(M10 证据)
