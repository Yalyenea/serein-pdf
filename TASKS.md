...
- [ ] 点选笔记 / sticky note（无选区 `M`）
- [ ] Caret 插入标记（插入内容写在评论里）
- [ ] Squiggly 强调线
...
- [ ] `UAT-59` 评论面板：打开后直接输入；点击阅读区或切换应用时关闭，键盘焦点正确返回阅读器（自动化测试进程未获前台焦点，待实机确认）
...
- [ ] `UAT-57` 左物理 `Cmd+1/2/3` 前三 tab；右物理 `Cmd+1/2/3/4` 四种原有阅读模式
- [ ] `UAT-58` 新增书籍模式且原双页模式行为不变：居中封面、后续左右 spread、封面与末尾孤页尺寸稳定；Fit Width 按实际 spread 完整放下、保留紧凑安全边距并双轴居中，手动缩小到完整可见时仍居中；普通状态每个横向手势翻一组，Continuous Turn 可持续逐组翻页；缩放后先惯性平移到边缘，反向翻页落在上一 spread 右端，翻页后的惯性不连跳；`C` 在当前布局内切换连续状态；`H` / `←` 与 `L` / `→` 左右翻页
