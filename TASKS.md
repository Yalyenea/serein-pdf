# TASKS

本文件是 [PROJECT.md](PROJECT.md) 的执行清单。后续开发默认以这里的任务状态为准推进。

## 1. 执行规则

- [ ] 每开始一个里程碑前，先确认依赖任务已完成。
- [ ] 每完成一个可见功能后，立即补最小验证。
- [ ] 所有状态管理优先落在 `DocumentStore` / `DocumentSession`，不要把业务状态散落到视图层。
- [ ] 垂直 tab 与水平标题栏 tab 必须共享同一套文档切换命令与状态模型。
- [ ] UI 决策始终遵守：极简、扁平、紧凑，不增加无必要控件。

## 2. 产品常量

- [ ] `TabPresentationMode` 只有两种：`verticalSidebar`、`horizontalTitlebar`
- [ ] 默认高亮颜色是“偏轻、低饱和、清晰”的粉色
- [ ] `a`：有选区时立即高亮；无选区时进入高亮模式
- [ ] `Esc`：退出高亮模式
- [ ] `i`：切换反色夜间模式
- [ ] `Cmd+S`：将当前文档未保存批注写回源 PDF
- [ ] 默认自动保存策略是 `10 min`
- [ ] 自动保存策略至少支持 `10 min` 和 `never`
- [ ] 水平 tab 模式必须复用标题栏，不单独新增一行 tab bar

## 3. Milestone 1: MVP 骨架

### 3.1 工程初始化

- [x] `M1-001` 初始化 macOS AppKit 工程
完成定义：工程可编译运行，包含主窗口入口。

- [x] `M1-002` 建立目录结构
完成定义：创建与 `PROJECT.md` 对齐的目录层级，至少包含 `App/`、`Core/`、`UI/`、`Features/`、`Tests/`。

- [x] `M1-003` 建立基础文件骨架
完成定义：创建 `MainWindowController`、`SplitViewController`、`DocumentStore`、`DocumentSession`、`ReaderViewController`、`OutlineViewController`、`VerticalTabsViewController`、`TitlebarTabsController` 的空实现。

### 3.2 主窗口与三栏布局

- [x] `M1-010` 实现主窗口控制器
完成定义：应用启动后显示主窗口，窗口样式支持后续 `unifiedCompact`。

- [x] `M1-011` 接入 `NSSplitViewController`
完成定义：主界面形成左、中、右三栏结构，左右栏可单独管理。

- [x] `M1-012` 放置中间阅读容器
完成定义：中栏显示 `ReaderViewController` 占位内容。

- [x] `M1-013` 放置右侧目录容器
完成定义：右栏显示 `OutlineViewController` 占位内容。

- [x] `M1-014` 放置左侧垂直 tab 容器
完成定义：垂直模式下左栏可以承载 tab 列表。

### 3.3 文档模型与会话管理

- [x] `M1-020` 定义 `DocumentSession`
完成定义：至少包含 `id`、`url`、`title`、`pdfDocument`、`currentPageIndex`、`zoomScale`、`lastReadPosition`、`outlineTree`、`isDirty`、`sidebarState`、`tabPresentationState`。

- [x] `M1-021` 定义 `TabPresentationMode`
完成定义：枚举仅含 `verticalSidebar`、`horizontalTitlebar`。

- [x] `M1-022` 实现 `DocumentStore`
完成定义：支持打开、关闭、切换、查找 active session、切换 tab 模式。

- [x] `M1-023` 设计视图层订阅方式
完成定义：确定 `DocumentStore` 到各控制器的更新路径，避免直接耦合。

### 3.4 PDF 打开与显示

- [x] `M1-030` 接入 `PDFKit`
完成定义：工程成功链接 `PDFKit`，`ReaderViewController` 可持有 `PDFView`。

- [x] `M1-031` 实现 `Command+O` 打开文件
完成定义：可选择本地 PDF 文件并创建 session。

- [x] `M1-032` 打开 PDF 后显示内容
完成定义：active session 的 `PDFDocument` 能在中间阅读区显示。

- [x] `M1-033` 打开多个 PDF
完成定义：重复打开多个 PDF 时可得到多个 session。

### 3.5 Tab 展示与切换

- [ ] `M1-040` 实现垂直 tab 列表
完成定义：左栏展示所有已打开文档、当前选中态、关闭入口。

- [ ] `M1-041` 实现标题栏水平 tab 容器
完成定义：水平模式下 tab 出现在标题栏区域，不新增内容区 tab bar。

- [ ] `M1-042` 实现 tab 模式切换
完成定义：可以在 `verticalSidebar` 与 `horizontalTitlebar` 间切换。

- [ ] `M1-043` 统一 tab 切换命令
完成定义：垂直和水平两套 UI 都调用同一套切换逻辑。

- [ ] `M1-044` 实现关闭 tab
完成定义：关闭当前或指定 session 后 active 文档行为正确。

### 3.6 Outline 与空状态

- [ ] `M1-050` 解析 PDF outline
完成定义：从 `PDFDocument` 读出目录层级。

- [ ] `M1-051` 定义 `OutlineNode`
完成定义：支持层级标题、目标页面、子节点。

- [ ] `M1-052` 用 `NSOutlineView` 显示目录树
完成定义：当前文档有目录时可展开和查看。

- [ ] `M1-053` 点击目录跳转页面
完成定义：点击目录节点后阅读区跳转到目标位置。

- [ ] `M1-054` 实现无目录空状态
完成定义：没有 outline 时显示明确、简洁的空状态。

### 3.7 侧栏与会话恢复

- [ ] `M1-060` 实现左右侧栏显隐
完成定义：不破坏中间阅读区布局。

- [ ] `M1-061` 保存已打开文档列表
完成定义：应用退出前记录当前 session 集合。

- [ ] `M1-062` 恢复上次打开会话
完成定义：重启后恢复文档列表、active session、tab 模式。

- [ ] `M1-063` 为 `DocumentStore` 编写单元测试
完成定义：覆盖打开、关闭、切换、模式切换、恢复逻辑。

### 3.8 M1 验收

- [ ] `M1-AC-01` 可以连续打开 3 个 PDF 并来回切换
- [ ] `M1-AC-02` 垂直 tab 与水平标题栏 tab 可自由切换
- [ ] `M1-AC-03` 两种 tab 模式下都能稳定切换文档
- [ ] `M1-AC-04` 右侧目录跟随当前文档更新
- [ ] `M1-AC-05` 无目录文档显示空状态
- [ ] `M1-AC-06` 重启后恢复文档列表与 tab 模式

## 4. Milestone 2: 阅读体验与快捷键

### 4.1 阅读基础能力

- [ ] `M2-001` 实现翻页
完成定义：支持上一页、下一页命令。

- [ ] `M2-002` 实现跳页
完成定义：可以跳转到指定页码。

- [ ] `M2-003` 实现缩放
完成定义：支持放大、缩小、重置缩放。

- [ ] `M2-004` 持久化阅读位置
完成定义：至少保存页码、缩放、滚动位置。

- [ ] `M2-005` 恢复阅读位置
完成定义：重新打开同一文档可回到之前位置。

### 4.2 搜索与最近文件

- [ ] `M2-010` 实现当前文档搜索
完成定义：支持输入关键词并定位结果。

- [ ] `M2-011` 实现最近文件存储
完成定义：记录最近打开列表，去重并排序。

- [ ] `M2-012` 实现最近文件重开
完成定义：可以从最近文件列表重新打开 PDF。

- [ ] `M2-013` 为 `ReadingPositionStore` 编写测试
完成定义：覆盖保存与恢复。

- [ ] `M2-014` 为 `RecentFilesStore` 编写测试
完成定义：覆盖去重、顺序和读取。

### 4.3 快捷键系统

- [ ] `M2-020` 建立统一快捷键入口
完成定义：阅读区快捷键不散落在多个 controller 中。

- [ ] `M2-021` 接入 `a` 快捷键
完成定义：为高亮逻辑预留统一命令入口。

- [ ] `M2-022` 接入 `Esc` 快捷键
完成定义：支持退出高亮模式。

- [ ] `M2-023` 接入 `i` 快捷键
完成定义：支持切换反色夜间模式。

- [ ] `M2-024` 快捷键冲突检查
完成定义：确认 `a`、`Esc`、`i` 与系统和文本输入上下文行为不冲突。

- [ ] `M2-025` 接入 `Cmd+S` 快捷键
完成定义：保存命令统一进入当前文档批注保存流程。

### 4.4 M2 验收

- [ ] `M2-AC-01` 同一文档重新打开后能恢复到之前阅读位置
- [ ] `M2-AC-02` 不同 tab 间切换时阅读状态互不污染
- [ ] `M2-AC-03` 搜索结果可定位到文档具体位置
- [ ] `M2-AC-04` 最近文件可用来重新打开历史 PDF
- [ ] `M2-AC-05` `i` 可稳定切换夜间模式，`Esc` 可退出高亮模式
- [ ] `M2-AC-06` `Cmd+S` 已接入当前文档保存命令

## 5. Milestone 3: 高亮与批注

### 5.1 默认高亮策略

- [ ] `M3-001` 定义默认轻粉色高亮
完成定义：确定默认粉色的具体颜色值，满足“偏轻、低饱和、清晰”。

- [ ] `M3-002` 将默认粉色接入 annotation 创建逻辑
完成定义：不额外选择颜色时，直接使用默认粉色。

- [ ] `M3-003` 固定色板支持
完成定义：提供黄色、绿色、粉色三种颜色，默认选中粉色。

### 5.2 高亮交互流

- [ ] `M3-010` 已有选区时按 `a` 立即高亮
完成定义：按键后立刻创建 annotation。

- [ ] `M3-011` 无选区时按 `a` 进入高亮模式
完成定义：阅读器进入可见但克制的高亮模式状态。

- [ ] `M3-012` 高亮模式下选中文字即自动高亮
完成定义：选区完成后自动生成默认粉色高亮。

- [ ] `M3-013` 按 `Esc` 退出高亮模式
完成定义：退出后选择文本不再自动高亮。

- [ ] `M3-014` 高亮模式状态可视反馈
完成定义：反馈足够清楚，但不破坏极简扁平风格。

### 5.3 批注管理

- [ ] `M3-020` 删除高亮
完成定义：选中已有高亮后可以删除。

- [ ] `M3-021` 建立批注脏状态
完成定义：创建或删除高亮后 session 进入未保存状态。

- [ ] `M3-022` `Cmd+S` 手动保存批注到 PDF
完成定义：只在用户显式保存时覆盖写回源 PDF。

- [ ] `M3-023` 自动保存策略模型
完成定义：至少支持 `after10Minutes` 和 `never`。

- [ ] `M3-024` 默认自动保存策略
完成定义：新会话默认采用 `after10Minutes`。

- [ ] `M3-025` 自动保存调度
完成定义：存在未保存批注时，按策略定时触发保存。

- [ ] `M3-026` 自动保存关闭逻辑
完成定义：当策略为 `never` 时，不触发自动保存。

- [ ] `M3-027` 未保存状态提示
完成定义：有未保存批注时 session / tab 状态正确反映。

- [ ] `M3-028` 保存失败处理
完成定义：写回失败时保留脏状态，并提供轻量提示。

- [ ] `M3-029` 高亮与保存测试
完成定义：覆盖 annotation 创建、颜色、删除、脏状态、手动保存、自动保存策略。

### 5.4 M3 验收

- [ ] `M3-AC-01` 默认高亮色符合“偏轻粉色、低饱和但清晰”
- [ ] `M3-AC-02` 选中文字后按 `a` 可以稳定添加高亮
- [ ] `M3-AC-03` 无选区时按 `a` 进入高亮模式
- [ ] `M3-AC-04` 高亮模式下选中文字即自动高亮
- [ ] `M3-AC-05` 按 `Esc` 可以退出高亮模式
- [ ] `M3-AC-06` 已有高亮可以删除
- [ ] `M3-AC-07` 高亮后文档进入未保存状态，但不会立即覆盖源文件
- [ ] `M3-AC-08` 按 `Cmd+S` 后当前文档批注被写回源 PDF
- [ ] `M3-AC-09` 自动保存默认值为 `10 min`
- [ ] `M3-AC-10` 自动保存可切换为 `never`
- [ ] `M3-AC-11` 保存后重新打开 PDF，高亮仍存在

## 6. Milestone 4: 夜间模式与视觉打磨

### 6.1 夜间模式

- [ ] `M4-001` 建立 `ThemeManager`
完成定义：提供夜间模式状态的单一入口。

- [ ] `M4-002` 实现基础深色 UI
完成定义：窗口、侧栏、空状态、控件都进入统一深色体系。

- [ ] `M4-003` 实现阅读区反色夜间模式
完成定义：文本 PDF 在夜间模式下明显更柔和可读。

- [ ] `M4-004` 绑定 `i` 快捷键到夜间模式切换
完成定义：切换逻辑统一走 `ThemeManager`。

- [ ] `M4-005` 处理扫描 PDF 的最小兼容方案
完成定义：即使无法完美反色，也能降低刺眼程度。

### 6.2 窗口与视觉

- [ ] `M4-010` 标题栏压缩
完成定义：启用 `unifiedCompact` 并最小化 toolbar 存在感。

- [ ] `M4-011` 优化水平标题栏 tab 样式
完成定义：与系统标题栏融为一体，不显臃肿。

- [ ] `M4-012` 优化垂直 tab 样式
完成定义：轻量、扁平、紧凑，当前选中态清晰。

- [ ] `M4-013` 优化边距与分栏拖动体验
完成定义：布局紧凑但不拥挤，拖动行为稳定。

- [ ] `M4-014` 清理多余阴影和装饰
完成定义：整体视觉不厚重、不拟物。

### 6.3 M4 验收

- [ ] `M4-AC-01` 深色模式下 UI 整体风格统一
- [ ] `M4-AC-02` `i` 可即时切换夜间模式，切换后状态同步正确
- [ ] `M4-AC-03` 夜间模式下文本 PDF 可读，不明显刺眼
- [ ] `M4-AC-04` 顶部空间占用显著低于常规文档应用
- [ ] `M4-AC-05` 整体视觉符合极简、扁平、紧凑目标

## 7. 当前推荐开工顺序

- [ ] `NOW-01` 完成 `M1-001` 到 `M1-023`
- [ ] `NOW-02` 完成 `M1-030` 到 `M1-044`
- [ ] `NOW-03` 完成 `M1-050` 到 `M1-063`
- [ ] `NOW-04` 做完 `M1-AC-*` 手测
- [ ] `NOW-05` 再进入 `M2` 的快捷键系统

## 8. 手测清单

- [ ] `UAT-01` 打开 3 个 PDF，分别在垂直与水平 tab 模式下切换
- [ ] `UAT-02` 切换文档时目录同步变化
- [ ] `UAT-03` 关闭当前 tab 后 active 文档正确
- [ ] `UAT-04` 重启后恢复文档、active session、tab 模式
- [ ] `UAT-05` 选中文字按 `a` 直接生成默认轻粉色高亮
- [ ] `UAT-06` 无选区按 `a` 进入高亮模式，`Esc` 退出
- [ ] `UAT-07` 高亮后未按 `Cmd+S` 时，源文件未被立刻覆盖
- [ ] `UAT-08` 按 `Cmd+S` 后重新打开 PDF，标注仍存在
- [ ] `UAT-09` 自动保存策略默认是 `10 min`，并可切换为 `never`
- [ ] `UAT-10` 按 `i` 切换夜间模式
