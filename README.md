...
- Internal-link previews fill a 6pt-corner panel without a header. Click links inside the preview to replace its page in place; Back/Forward arrows in the upper-left corner restore previous preview pages, scroll positions, and zoom. Following a new link after going back clears the forward branch. The preview copies one page at a time and suppresses comment popups while preserving the source annotations. Option-click in the reader, or the upper-right Jump icon for the current preview target, navigates the main reader and records its history.
