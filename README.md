...
- Highlights with comments preview on hover (delayed, suppressed when the same item is selected in the Annotations pane). Each multiline comment has one icon; identical comments in older Serein groups are consolidated on open. Clicking the icon, double-clicking the highlight, `cmd+option+m`, or the reader context menu opens Serein's compact comment panel, with 6pt corners, tight bottom spacing, and an editor that grows with its contents up to a scrollable limit. The panel and hover preview follow the current theme. The Annotations list supports inline edit, right-click recolor/delete/copy, and keyboard delete; Markdown export remains page-grouped as snippet + comment
- Internal-link previews fill a 6pt-corner panel without a header, with a single jump icon in the upper-right corner. The preview copy suppresses comment popups while preserving the source PDF's annotations. Option-click or the jump icon navigates to the reference and records history.
