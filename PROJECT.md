...
| 评论交互 | 有评论的高亮 / 下划线 / 删除线 hover 延迟显示预览(无评论不弹;右栏同条已选中时抑制);预览与编辑共用批注旁的无边框 `NSPanel`,单击图标或预览原地编辑,保持宽度与靠近图标的一角,避开图标;鼠标跨入卡片有 150ms 宽限,编辑时移出不关闭;双击批注 / `M` / 右键也可编辑,不强制打开右栏;应用接管批注点击与右键,不打开 PDFKit 黄色编辑器;评论标记为高亮色小圆点,放在命中旁的空白处(优先行右 / 页边,避开正文与其它标记),并隐藏 PDFKit 原生编号角标;卡片采用 6pt 圆角与紧凑底边距,空评论最小输入高度 20pt,长评论随内容增高并在上限后滚动;底部右侧显示 `Save ⌘↩`,Esc 保留取消操作但不显示提示;卡片随主题刷新;右栏为全高评论流,摘要最多 4 行、评论最多 8 行,轻选中态 + 细色条,不自动选中首条,支持行内编辑、右键改色 / 删除 / 复制,跳转用短时 pulse 而非虚线选区 |
...
| 文本点选 | 单击设置起点，`Shift` + 单击选择到终点，支持反向、跨页和行首尾空白；连续 `Shift` + 单击调整终点。起点按阅读区独立保存，切换文档或移出窗口时清除；通过本地鼠标事件监听处理，保留 PDFKit 原生拖选与双击选词。 |
...
- `L`:切换当前 PDF 的水平平移锁定(保持当前 X 位置并禁止左右滑动与缩放手势；缩放命令及页面 / 文本适配仍可使用，完成后锁定新的 X 位置；竖直滚动与翻页不受影响;按焦点 pane 操作,各文档独立保存并跨启动恢复;书籍模式不启用)
- 单页 / 双页非连续模式下,触控板上下滑动先在页内滚动,到边缘后通过与 `J` / `K` 相同的路径翻页;每次手势最多翻一页 / 一组双页,翻页后消耗剩余手势与惯性,惯性本身不触发翻页。
...
| `scaleMode` | `fitWidth` / `fitHeight` / `manual` |
| `zoomScale: CGFloat` | 缩放比例 |
| `isHorizontalPanLocked: Bool` | 当前文档的水平平移锁定,随阅读状态保存 |
