...
做一个你自己愿意天天打开的 macOS PDF 阅读器，而不是一个“大而全”的 PDF 软件。

产品目标用一句话概括：

> 比 Skim 更现代，比 PDF Expert 更轻，比 Preview 更有个性。
...
| 优先级 | 能力 | 说明 |
|---|---|---|
| P0 | 打开多个 PDF | 支持多文档并行阅读 |
| P0 | tab 布局自由切换 | 支持左侧垂直 tab 与标题栏水平 tab 两种模式 |
| P0 | 右侧查看目录 | 当前文档 outline 清晰可跳转 |
| P0 | 中间沉浸阅读 | 内容最大化，减少顶部干扰 |
| P0 | 基础高亮 | 默认轻粉色高亮，支持键盘驱动的快速标注 |
| P0 | 夜间模式 | 支持 `i` 快捷切换反色夜间模式 |
| P0 | 尽量薄的窗口头部 | 降低标题栏和 toolbar 存在感 |
| P0 | 极简扁平紧凑视觉 | 控件克制、层级简洁、留白受控 |
...
| 阅读器 | PDF 显示、滚动、缩放、翻页、跳页 |
| 目录 | 解析并展示 PDF outline，支持点击跳转 |
| 状态恢复 | 最近文件、恢复上次打开文档、记住每个文档阅读位置 |
| 搜索 | 当前文档文本搜索 |
| 批注 | 文本高亮、删除高亮、保存批注、默认轻粉色高亮、脏状态后手动保存 |
| 主题 | UI 深色主题、基础夜间阅读模式、反色切换 |
| 快捷键 | `a` 进入或执行高亮、`Esc` 退出高亮模式、`i` 切换夜间模式、`Cmd+S` 保存批注 |
...
| 编号 | 任务 | 说明 |
|---|---|---|
| M2-1 | 翻页与跳页能力 | 菜单、快捷键、程序接口统一 |
| M2-2 | 缩放状态管理 | 每个 session 独立保存 |
| M2-3 | 文本搜索 | 基于 `PDFDocument` / `PDFSelection` 能力 |
| M2-4 | 最近文件列表 | 记录最近打开项 |
| M2-5 | 阅读位置持久化 | 页码、缩放、滚动位置 |
...
| `ReadingPositionStore` | Unit Test | 状态持久化读写 |
